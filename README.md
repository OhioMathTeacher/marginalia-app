...
Pick any provider in the **🤖 AI Setup** menu. The recommended path is a
**local model via Ollama** — fully private, no key, no per-token bill,
and aligned with what Marginalia is for: your reading and writing stay
on your own machine.

### Local model (recommended) — Ollama + qwen2.5:3b

[Ollama](https://ollama.com/) is a one-binary local LLM runner. Install it,
pull a small model, and Marginalia auto-discovers it at `localhost:11434`.
...
Applications), then run `ollama pull qwen2.5:3b` in Terminal.

On Windows: download the installer from [ollama.com/download](https://ollama.com/download),
then run `ollama pull qwen2.5:3b` in PowerShell.

`qwen2.5:3b` is ~2 GB and runs comfortably on most laptops (including
older machines without a discrete GPU). Open Marginalia → **🤖 AI Setup**
→ pick **Local (Ollama)** → select `qwen2.5:3b`. That's it.

Want better answers on a beefier machine? `ollama pull qwen2.5:7b` (~5 GB,
needs ~16 GB RAM to be comfortable) or `gemma3:4b` (~3 GB, same family
as larger Gemma models you can grow into).

### Cloud model (alternative) — bring your own key

If you'd rather use a hosted model, Marginalia also supports Anthropic
Claude, Google Gemini, and Groq. Paste your API key in **🤖 AI Setup**.
Keys are stored only in your browser's local storage and never written
into the source.
